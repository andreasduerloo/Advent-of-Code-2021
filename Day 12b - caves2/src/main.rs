        ...
        for index in 0..route_vec.len() {
            if route_vec[index][route_vec[index].len() - 1] != "end" && route_vec[index][route_vec[index].len() - 1] != "to_remove" { // This route is not finished, make a route for each of its neighbors, then remove the original
                ...
                for neighbor_index in 1..cave_vec[cave_index].len() {
                    if cave_vec[cave_index][neighbor_index] == "start" {
                        continue;
                    }
                    if cave_vec[cave_index][neighbor_index] == cave_vec[cave_index][neighbor_index].to_lowercase() && ! passed_twice(&route_vec[index]) {
                    ...
                    } else if cave_vec[cave_index][neighbor_index] == cave_vec[cave_index][neighbor_index].to_lowercase() && passed_twice(&route_vec[index]) && ! route_vec[index].contains(&cave_vec[cave_index][neighbor_index]) {
...
fn passed_twice(input_vec: &Vec<&str>) -> bool {
    for cave in input_vec {
        if *cave == "start" || *cave == "end" {
            continue;
        } else if *cave == cave.to_lowercase() {
            if count_cave(cave, input_vec) > 1 {
                return true;
            }
        } else {
            continue;
